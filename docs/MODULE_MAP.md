...
| 13 | Relativistic Camera Model           | `src/relativity/`                        | production ready        | `aberrateDirection` / `dopplerFactor` / `searchlightFactor` + `RelativityParams` + `Observer`; RR_HD host + device callable; tested host-side and integrated into both CUDA + OptiX raygen / closest-hit. The project's core differentiator. |
